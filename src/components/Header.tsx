    ...
    <header className="bg-white text-[#112a44] sticky top-0 z-50 shadow-md border-b border-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6 text-sm font-medium">
          <NavLink to="/" end className={({ isActive }) => isActive ? 'text-[#f76a0c] font-semibold' : 'text-[#112a44] hover:text-[#f76a0c] transition-colors'}>
            Home
          </NavLink>

          <NavLink to="/residential-pest-control" className={({ isActive }) => isActive ? 'text-[#f76a0c] font-semibold' : 'text-[#112a44] hover:text-[#f76a0c] transition-colors'}>
            Residential
          </NavLink>

          <NavLink to="/commercial-pest-control" className={({ isActive }) => isActive ? 'text-[#f76a0c] font-semibold' : 'text-[#112a44] hover:text-[#f76a0c] transition-colors'}>
            Commercial
          </NavLink>

          {/* Services mega menu */}
          <div className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
            <button className="text-[#112a44] hover:text-[#f76a0c] transition-colors flex items-center gap-1">
              Services
                ...
                <div className="grid grid-cols-3 gap-6">
                  {megaMenu.map(group => (
                    <div key={group.category}>
                      <p className="text-xs font-bold text-[#f76a0c] uppercase tracking-widest mb-3">{group.category}</p>
                        ...
                        {group.items.map(item => (
                          <li key={item.path}>
                            <Link
                              ...
                              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-[#112a44] hover:bg-[#faf8ee] hover:text-[#f76a0c] transition-colors group"
                            >
                              <span className="w-1.5 h-1.5 rounded-full bg-[#f76a0c] flex-shrink-0" />
          ...
          <NavLink to="/new-articles" className={({ isActive }) => isActive ? 'text-[#f76a0c] font-semibold' : 'text-[#112a44] hover:text-[#f76a0c] transition-colors'}>
            Blog
          </NavLink>
          <NavLink to="/contact-us" className={({ isActive }) => isActive ? 'text-[#f76a0c] font-semibold' : 'text-[#112a44] hover:text-[#f76a0c] transition-colors'}>
            Contact
          </NavLink>
        </nav>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-2 text-sm font-medium shadow-lg">
          <Link to="/" className="text-[#112a44] py-2 hover:text-[#f76a0c]" onClick={() => setMobileOpen(false)}>Home</Link>
          <button
            className="text-[#112a44] py-2 hover:text-[#f76a0c] flex items-center justify-between"
            ...
            <div className="pl-4 flex flex-col gap-1">
              {allServices.map((s) => (
                <Link
                  key={s.path}
                  to={s.path}
                  className="text-gray-500 py-1.5 hover:text-[#f76a0c]"
          ...
          <Link to="/residential-pest-control" className="text-[#112a44] py-2 hover:text-[#f76a0c]" onClick={() => setMobileOpen(false)}>Residential</Link>
